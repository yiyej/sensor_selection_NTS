...
def distance_sklearn_metrics(z=None, k=4, metric='euclidean', d=None):
    """Compute exact pairwise distances."""
    assert z is not None or d is not None
    if d is None:
...
def adjacency(dist, idx, k_self_tuning = None):
    ...
    # Weights.
    if k_self_tuning is not None:
        sigma = dist[:, k_self_tuning - 1] #self tuning
        dist = dist**2 / sigma[idx]
        dist = np.exp(- dist / sigma.reshape(M,1))
    else:
